  ...
  return (
    <div className="flex items-center justify-between px-6 py-4 bg-white border-b border-gray-200 sticky top-0 z-[100]">
      { pathname === '/' ?
        ...
        <>
          <button className="w-9 h-9 rounded-full bg-gray-50 text-gray-900 flex items-center justify-center text-base hover:bg-amber-50 transition-all duration-200 flex-shrink-0" onClick={() => navigate(-1)}>
            ←
          </button>
          <div className="flex-1">
            <div className="font-serif text-base text-gray-900">{title}</div> 
            <div className="text-xs text-gray-400 mt-0.5">Salon Baraka · Aujourd'hui, mercredi 3 juin</div>
          </div>
        </>
        ...
        { pathname === '/' ?
          <>
            <div className="relative">
              ...
              <div className="absolute top-1.5 right-1.5 w-2 h-2 rounded-full bg-amber-600 border-2 border-white"></div>
            </div>
            <button className="w-9 h-9 rounded-full bg-gray-50 text-gray-500 flex items-center justify-center text-base hover:bg-amber-50 hover:text-amber-700 transition-all duration-200">
              ⚙️
            </button>
